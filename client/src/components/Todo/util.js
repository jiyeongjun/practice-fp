...
// Create
const addFn = async () =>
  !$qs(".todo__form__input").value
    ? await UiHelper.alert("할 일을 입력해주세요!") // 작성글 없는 경우 안내창을 띄워준다.¡
    : Suspense(
      // 작성글 있는 경우
      go(
        $qs("#todo__form__input").value, // 작성글을
...
const editFn = ({ currentTarget }) => {
  $qs('.todo__list__item__message.hidden') && // 수정 중인 요소가 있을 경우만 실행하고,
  go(
    $qs('.todo__list__item__message.hidden'), // 수정 중인 요소의
    $closest('.todo__list__item'), // 아이템을 찾아서
...
const saveFn = ({ currentTarget }) => {
  UiHelper.Loading(
    go(
      ...
      $find('.todo__list__item__edit'),
      $val, // 현재 요소의 값을 찾아서
      title => ({
        todo_id: parseInt($closest('.todo__list__item', currentTarget).dataset.todoId),
        title,
        is_completed: $hasClass('todo__list__item__check', $qs(".todo__list__item")),
